...
add_dmg = str_mod + item_dmg; %add'l damage on every attack


add_atk = str_mod + item_atk + prof + cha*sacred_weapon; %to-hit bonus
...
if smite
avg_dmg = avg_dmg + 4.5*(1+smite_level+undead_fiend); %add smite damage when applicable
fprintf('Sorghum''s Average DPR against an AC of %d with a level %d smite is: %f\n', enemy_AC, smite_level, avg_dmg)
else
    fprintf('Sorghum''s Average DPR against an AC of %d is: %f\n', enemy_AC, avg_dmg);
end

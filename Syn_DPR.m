...
add_dmg = dex_mod + item_dmg + (stance * stance_dmg); %add'l damage on every attack
add_atk = dex_mod + item_atk + prof; %to-hit bonus
...
adv_crit_prob = 1-anti_crit^3;
adv_mini_prob = 1-anti_mini^3;
adv_reg_prob = 1 - (adv_mini_prob + adv_crit_prob);
...
adv_avg_dmg = (adv_reg_dmg + adv_mini_dmg + adv_crit_dmg);

%=================================================
%probability of having advantage on each attack
avg_dmg = avg_dmg + familiar*adv_avg_dmg;
for i = 2:num_attacks
   adv_chance = 1-(19/20)^(i+1);
 avg_dmg = adv_avg_dmg * adv_chance + avg_dmg;
end

fprintf('Syn''s Average DPR against an AC of %d is: %f\n', enemy_AC, avg_dmg)
